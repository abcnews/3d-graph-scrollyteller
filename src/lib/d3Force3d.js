require("es6-map/implement"); // IE11 Map Polyfill
...
(function(global, factory) {
  ...
  function center(x, y, z) {
    ...
    function force() {
      ...
      for (
        sx = sx / n - x, sy = sy / n - y, sz = sz / n - z, i = 0;
        i < n;
        ++i
      ) {
        node = nodes[i];
  ...
  function collide(radius) {
    ...
    function force() {
      ...
      for (var k = 0; k < iterations; ++k) {
        tree = (nDim === 1
          ? d3Binarytree.binarytree(nodes, x)
          : nDim === 2
          ? d3Quadtree.quadtree(nodes, x, y)
          : nDim === 3
          ? d3Octree.octree(nodes, x, y, z)
          : null
        ).visitAfter(prepare);
  ...
  function find(nodeById, nodeId) {
    ...
    if (!node) throw new Error("Force node missing: " + nodeId);
    return node;
  }
    ...
    function initializeNodes() {
      for (var i = 0, n = nodes.length, node; i < n; ++i) {
        ...
        if (
          ...
          var radius =
              initialRadius *
              (nDim > 2 ? Math.cbrt(i) : nDim > 1 ? Math.sqrt(i) : i),
  ...
  function manyBody() {
    ...
    function force(_) {
      var i,
        n = nodes.length,
        tree = (nDim === 1
          ? d3Binarytree.binarytree(nodes, x$1)
          : nDim === 2
          ? d3Quadtree.quadtree(nodes, x$1, y$1)
          : nDim === 3
          ? d3Octree.octree(nodes, x$1, y$1, z$1)
          : null
  ...
  function z$2(z) {
    ...
    force.z = function(_) {
      return arguments.length
        ? ((z = typeof _ === "function" ? _ : constant(+_)),
          initialize(),
          force)
        : z;
    };
